...
module EnumerateOneAST = struct
  ...
  let expansions_for_hole ~hole ~unification ~label_arities ~vars ~introduce_blanks
      ~must_be_leaf =
    match hole.kind with
    | Blank label_only -> if must_be_leaf then [ Types.Hole hole ] else [ Types.Hole hole ]
    | TypeHole ->
  ...
  let rec commit c ~unification ~introduce_blanks ~fast_forward_blanks ~size_bound
    ...
    if SearchState.no_holes c then Sequence.singleton c
      ...
      if fast_forward_blanks then
        Sequence.of_list (Option.to_list (fast_forward c))
      else Sequence.singleton c
    else if Int.equal size_bound 0 then
      Sequence.of_list (Option.to_list (fast_forward c))
    else
      let holes =
        c.SearchState.types
      ...
      let i_to_fill, (hole, depth) =
        List.min_elt holes ~compare:(fun (_, (_, d1)) (_, (_, d2)) ->
            Int.compare d1 d2)
        |> Option.value_exn
      in
      let expansions =
        expansions_for_hole ~hole ~unification ~label_arities:c.label_arities
          ~vars:(Set.length (Types.variables (List.nth_exn c.types i_to_fill)))
          ~introduce_blanks ~must_be_leaf:(size_bound <= 1 || depth > hard_depth_bound)
      in
      ...
      |> Sequence.concat_map ~f:(fun (cand, u) ->
             Logger.count logging_seed "[commit] candidates";
             commit cand ~unification:u ~introduce_blanks ~fast_forward_blanks
               ~size_bound:(size_bound - 1) ~hard_depth_bound ~logging_seed)
end
