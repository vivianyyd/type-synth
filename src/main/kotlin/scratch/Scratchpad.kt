...
fun f(line: String): String {
    return line
        .split(": ")
        .map {
            val noHoles = it.replace(Regex("_[0-9]+_"), "_").replace(Regex("☐[0-9]+"), "_")
            val regex = Regex("V\\d+")

            val vars = mutableMapOf<String, String>()
            var counter = 0
            fun g(x: String): String = vars.getOrPut(x) { "V${counter++}" }

            val result = regex.replace(noHoles) { match -> g(match.value) }
            result
        }
                ...
                if ("Now:" in line) {
                    writer.println(
                        line
                            .replace("b: L0[], ", "")
                            .replace(
                                "dbb: L1[_, _], dbi: L1[_, _], dib: L1[_, _], dii: L1[_, _], i: L8[], ",
                                ""
                            )
                    )
                }
...
//    val (changeInd, leftmostNode) = c.withIndex().firstOrNull { (i, it) -> it.containsHole() } ?:
// return listOf(c)
//
