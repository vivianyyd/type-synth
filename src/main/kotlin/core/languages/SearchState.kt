...
/**
 * SearchNodes are hashable. All but holes are functional and immutable. Holes mutate when they
 * count conflicts, but they are normal rather than data classes, so they are physical equals
 */
sealed interface SearchNode<L : Language> {
    ...
    /** Holes which can be filled in the type. In particular, excludes blanks. */
    ...
    /**
     * TODO not sure if the invariant here should be that num holes = 0 iff full. I think we need
     * one metric for no more holes *to fill* , one metric for it's actually a full type with no
     * holes
     */
    fun full(): Boolean
    ...
    /**
     * Optional for correctness, but lets us reject bad candidates. While a function certainly
     * return L[a] where a is unbound, it cannot simply return any a.
     */
    fun noFreshSoleVarOnRHS(): Boolean =
        when (this) {
            is NArrow -> {
...
sealed class Branch<L : Language>(open val params: List<SearchNode<L>>) : SearchNode<L> {
    ...
    private val holes by lazy { params.sumOf { it.holes() } }
    ...
    private val depth by lazy { 1 + (params.maxOfOrNull { it.depth() } ?: 0) }
...
data class NArrow<L : Language>
constructor(override val params: List<SearchNode<L>>, val contributesToDepth: Boolean) :
    Branch<L>(params) {
    val l = params[0]
    val r = params[1]
    ...
    constructor(
        l: SearchNode<L>,
        r: SearchNode<L>,
        contributesToDepth: Boolean
    ) : this(listOf(l, r), contributesToDepth)

    override fun depth() = depth

    private val depth by lazy { max(l.depth(), r.depth()) + (if (contributesToDepth) 1 else 0) }
    ...
    override fun replaceWithAll(hole: Hole<L>, nodes: List<SearchNode<L>>): List<SearchNode<L>> {
        ...
        return if (newL.isNotEmpty()) newL.map { NArrow(it, r, contributesToDepth) }
    ...
    override fun dfsLeftExpansions(
        ...
        val left =
            l.dfsLeftExpansions(unification, vars, nextBound).map { (node, commit) ->
                NArrow(node, r, contributesToDepth) to commit
            }
        val right =
            if (left.isEmpty() || (left.toSet().size == 1 && left.first().first.l == l))
    ...
    override fun dfsPriorityExpansions(
        ...
        if (l.priority() >= r.priority()) {
            ...
            val left =
                if (right.isEmpty() || (right.toSet().size == 1 && right.first().first.l == l))
                    l.dfsPriorityExpansions(unification, vars, nextBound).map { (node, commit) ->
                        NArrow(node, r, contributesToDepth) to commit
                    }
                else listOf()
...
sealed interface Leaf<L : Language> : SearchNode<L> {
    ...
    override fun dfsLeftExpansions(
        ...
        vars: Int,
        recursionBound: Int?
    ): List<Pair<SearchNode<L>, Commitment<L>>> = listOf(this to null)
...
sealed class Hole<L : Language> : SearchNode<L> {
    ...
    override fun replace(hole: Hole<L>, node: SearchNode<L>): SearchNode<L> =
        if (hole == this) node else this

    override fun replaceWithAll(hole: Hole<L>, nodes: List<SearchNode<L>>) =
        if (hole == this) nodes else listOf(this)
    ...
    fun instantiations(): List<Instantiation<L>> =
        TODO("No longer supported bc it takes too much memory")
...
data class Candidate<L : Language>(val names: List<String>, val types: List<SearchNode<L>>) {
    ...
    constructor(
        names: List<String>,
        types: List<SearchNode<L>>,
        deps: Map<ParameterNode, Dependency>
    ) : this(names, types) {
        this.deps = deps
    }
    ...
    val assocList by lazy { names.zip(types) }

    val holes by lazy { types.sumOf { it.holes() } }

    fun depth() = depth

    private val depth by lazy { types.maxOf { it.depth() } }
        ...
        return deps!!.all { (param, dep) ->
            ...
            !paramNode.full() ||
                    when (dep) {
                        ...
                        is Only ->
                            paramNode.variableNames().size == 1 &&
                                    paramNode.variableNames().first() == dep.v
                    }
        }
    }
    ...
    fun fastForward(unification: (Candidate<L>) -> Unification<L>): Candidate<L>? {
        ...
        do {
            ...
            val commitments =
                curr.types.map { t ->
                    t.listHoles().map { it to it.fastForward(u, t.variableNames().size) }
                }
            curr =
                Candidate(
                    names,
                    curr.types.zip(commitments).map { (t, commits) ->
                        commits.fold(t) { acc: SearchNode<L>,
                                          commitment: Pair<Hole<L>, SearchNode<L>?> ->
