...
class EagerUnification<L : Language>(
    ...
    override fun spawnAndRefine(refinements: List<Pair<Hole<L>, SearchNode<L>>>): Unification<L> {
        ...
        fun refine(n: SearchNode<L>): SearchNode<L> =
            when (n) {
                is Hole -> ref[n] ?: n
                is Leaf -> n
                ...
                is ConcreteL ->
                    ConcreteL(
                        n.id,
                        n.params.map { refine(it as SearchNode<L>) } as List<SearchNode<Concrete>>)
                            as SearchNode<L>
            }
    ...
    fun unify(param: ConstraintType<L>, arg: ConstraintType<L>): List<Binding<L>>? =
        when (param) {
            is Substitutable ->
                if (param in arg.substitutable()) null else listOf(Binding(param, arg))
            is CTypeConstructor ->
                when (arg) {
                    is CTypeConstructor -> {
                    ...
                    is Substitutable -> // for example, a function expects argument (int -> int) and
                        // we pass ('a -> 'a)
                        if (arg in param.substitutable()) null else listOf(Binding(arg, param))
                    InitConstrV -> listOf()
                    is Instantiation -> {
        ...
        return when (t) {
            ...
            is CTypeConstructor -> {
                ...
                (when (t) {
                    is CArrow -> CArrow(p)
                    is ConcreteConstrL ->
                        ConcreteConstrL(t.label, p as List<ConstraintType<Concrete>>)
                    InitConstrL,
                    ElabConstrL,
                    is ElaboratedConstrL -> error("hasSubstitutable should have been false")
                }
                        as ConstraintType<L>)
            }
