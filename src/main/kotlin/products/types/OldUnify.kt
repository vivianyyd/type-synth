...
object OldUnify {
    ...
    private fun unify(a: Type, b: Type, map: Context): Pair<Type, Context> =
        when (a) {
            is Variable -> {
                when (b) {
                ...
                when (b) {
                    ...
                    is Function -> {
                        ...
                        Pair(
                            ...
                            outerContext // If unifying inputs of (a->a)->a and (int->int)->int,
                            // need to store a:=int
                        )
                    } // TODO think about variable shadowing. I think a->(a->a) is fine! what about
                    // unifying a-> (a->b) if they have diff names. should each type just have
                ...
                when (b) {
                    ...
                    is LabelNode -> {
                        if (a.label != b.label) Pair(Error(a, b, ErrorCategory.LABEL_MISMATCH), map)
                            ...
                            val params =
                                a.params.indices.map {
                                    val (param, newMap) = unify(a.params[it], b.params[it], currMap)
                                    if (param is Error && error == null)
                                        error = Pair(param, currMap)
                                    currMap = newMap
                                    param
                                }
    ...
    fun apply(f: Type, arg: Type): Type {
        // TODO figure out what to output if f is a hole. alternatively just always enum functions
        // first but sometimes cyclic
    ...
    private fun resolve(t: Type, map: Context): Pair<Type, Context> {
        return when (t) {
            ...
            // TODO ^ I think we want to keep variables not in the Context, instead of throwing an
            // error
