...
data class N(val name: String) : OldSymTypeB {
    override fun constructSketch(): String =
        throw UnsupportedOperationException("Bad code style, fixme later")
}
...
fun applyBinding(t: OldSymTypeB, varId: Int, tId: Int, sub: OldSymTypeB): OldSymTypeB =
    when (t) {
        is CL,
        L,
        is VB,
 ...
 * incompatible.
 */
fun unify(param: OldSymTypeB, arg: OldSymTypeB): List<Binding>? =
    when (param) {
        ...
        is CL ->
            when (arg) {
                ...
                is F -> null
                L,
                is VL -> listOf() // TODO I think we shouldn't see VLs here ever
                is VB,
                is VR,
        ...
        is F ->
            when (arg) {
                ...
                is F -> {
                    ...
                    val riteBindings =
                        leftBindings
                            ?.let { applyBindings(param.rite, it) }
                            ?.let { unify(it, arg.rite) }
