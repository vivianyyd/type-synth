...
class State(query: Query) {
     ...
     *
     * TODO: This is only intended for manually setting nullaries to Label; make this nicer
     *   encapsulated later
     */
     ...
     */
    fun exprToChoice(ex: Example): Choice? =
        when (ex) {
            ...
            is App -> {
                ...
                if (fn == null) null
                    ...
                    if (!options.any { it is Variable || it is Function })
                        throw Exception("UNSAT!" + printState())
    ...
    /**
     * Ensure the options for [choice] are a subset of types satisfying [predicate]. If options for
     * [choice] are empty/not yet enumerated, populates them.
     */
    ...
    private fun prune(choice: Choice, predicate: Predicate<SymTypeA>) {
        ...
        // TODO Somebody needs to catch this. Or do something more idiomatic and return special type
        // or boolean
        if (empty(choice)) throw Exception("UNSAT!")
    }

    private fun allOptions(parent: Parent?) =
        listOf(Variable(parent), Label(parent), Function(parent = parent))
...
class SymTypeABuilder(val query: Query) {
    ...
    private fun antiunify(trees: List<List<SymTypeA>>, param: List<SymTypeA>): List<SymTypeA> {
        ...
        if (trees.any { it.singleton<Hole>() })
            return antiunify(trees.filter { !(it.size == 1 && it[0] is Hole) }, param)
        if (trees.any { it.singleton<Function>() } && trees.any { it.singleton<Label>() })
            listOf(Variable())
        ...
        if (trees.all { it.singleton<Function>() }) {
            return if (param.any { it is Function }) {
                ...
                notFn +
                        Function(
                            antiunify(trees.map { (it[0] as Function).left }, (fn[0] as Function).left)
                                .toMutableList(),
                            antiunify(trees.map { (it[0] as Function).rite }, (fn[0] as Function).rite)
                                .toMutableList()
                        )
            } else
                param +
                        Function(
                            antiunify(trees.map { (it[0] as Function).left }, listOf(Hole()))
                                .toMutableList(),
                            antiunify(trees.map { (it[0] as Function).rite }, listOf(Hole()))
                                .toMutableList()
                        )
        }
        return param // TODO
    ...
    fun deepen(): State {
        ...
        paramToArgs.forEach { (param, args) ->
            ...
            // antiunify arguments, then use it to fill in param - second step is not quite
            // antiunif?
            // actual type should be intersected with result of au unless hole then = returned thing
            // and unless fn
            // then recurse??
    ...
    private fun patchEmptyLists() {
        fun patch(t: SymTypeA, rightmost: Boolean) {
            when (t) {
                is Function -> {
                    if (t.left.isEmpty()) t.left.add(Hole()) else t.left.map { patch(it, false) }
                    if (t.rite.isEmpty()) {
                        if (rightmost) t.rite.add(Hole()) else t.rite.add(Hole())
    ...
    private fun validArg(fn: Example, arg: Example) {
        ...
        if (lhsSingle is Function)
            s.ensureSubset(rhs) { t: SymTypeA -> t is Function || t is Variable }
    ...
    private fun readAllExamples() {
        ...
        // Hack for multi arg fns: need to read partial applications in order to deepen the tree one
        // node at a time
