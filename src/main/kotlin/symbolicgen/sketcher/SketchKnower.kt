...
fun main() {
    ...
    val encoding = SketchKnower(query, b, oracle)
    ...
    val runSketch = false
    if (runSketch) {
        File(sketch).printWriter().use { it.println(encoding.sketchInput()) }
        out = callSketch()
    ...
    println(encoding.parse("_cons", out))
    val firstCandidate = encoding.parse(/*TODO encoding.sk("cons")*/"_cons", out).constructSketch()
    println(firstCandidate)

//    val nextQuery = encoding.sketchInput() + "\n" + "harness void neq() { assert (_cons() != $firstCandidate); }"
//    File(sketch).printWriter().use { it.println(nextQuery) }
//    val newOut = callSketch()
//    File(sketchOut).printWriter().use { it.println(newOut) }
//    println(encoding.parse(/*TODO encoding.sk("cons")*/"_cons", newOut))
}
