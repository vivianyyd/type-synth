...
fun applyOrError(fn: Function, arg: Type): OrError<Type> {
    ...
    return if (bindings is Err) Err(bindings.error)
    else Ok(applyBindings(fn.rite, (bindings as Ok).result))
}
        ...
        is Function ->
            when (arg) {
                ...
                is Function -> {
                    ...
                    val riteBindings =
                        if (leftBindings is Err) leftBindings
                        else
                            unifyOrError(
                                applyBindings(param.rite, (leftBindings as Ok).result), arg.rite
                            )
                    riteBindings.bind {
                        Ok((leftBindings as Ok).result + (riteBindings as Ok).result)
                    }
                }
            }
        is LabelNode ->
            when (arg) {
                ...
                is LabelNode -> {
                    ...
                    else if (param.params.size != arg.params.size)
                        Err(ErrorCategory.PARAM_QUANTITY_MISMATCH)
                    else
                        param.params.zip(arg.params).fold(Ok(listOf())) { acc: OrError<List<Binding>>,
                                                                          (p, a) ->
...
sealed interface OrError<T> {
    fun bind(f: (T) -> OrError<T>): OrError<T> =
        when (this) {
            is Err -> this
            is Ok -> f(this.result)
        }
