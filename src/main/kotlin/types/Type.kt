...
fun Type.numParams(): Int =
    when (this) {
        is LabelNode,
        is Variable -> 0
        is Function -> 1 + this.rite.numParams()
        is Error,
        is TypeHole -> throw Exception("What is this")
    }

fun Type.recursiveNumChildHoles(): Int =
    when (this) {
        is ChildHole -> 1
...
fun Type.directChildHoles(): Boolean =
    when (this) {
...
sealed class AbstractType : Type {
    ...
    override val height: Int by lazy { 1 + (children.maxOfOrNull { it.height } ?: 0) }
}
...
/**
 * Unifies with everything, producing the other type. Represents a hole/tree not yet completely
 * enumerated.
 *
