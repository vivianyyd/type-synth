...
class OldSymTypeBSketcher(val query: Query, private val state: State, private val oracle: Oracle) {
    ...
    fun nextQuery(sketch: String, round: Int) =
        sw.addBanned(SymbolicSketchParser(sketch).parseAll.first, round)
    ...
    private inner class SymbolicSketchWriter {
        ...
        private fun chooseFromOptions(
            ...
            // TODO not sure if this is right. we didn't see anything that forced us to make this a
        ...
        private fun generator(name: String) {
            ...
            if (nullary(name))
                w.singleLineBlock(
                    header, "return new ConcreteLabel(dummy=${oracle.dummy(Name(name))})"
                )
            else
                w.block(header) {
                    val options = state.read()[name]!!
                    w.lines(
                        listOf(
                            "Type root",
                            // TODO canBeFresh need not be in Sketch, it is a property of the tree
                            // shape not choices
