...
data class L(
    ...
    ) : this(label, (0 until numParams).map { mutableListOf(Hole(constraint)) }, id, constraint)
...
class ConcreteEnumerator(
    ...
    /**
     * While we start with all functions flattened, we might enumerate functions with functions as
     * outputs
     */
    // Don't memoize this once, since we want to create new objects
    private fun filler(name: String, param: Int, constraint: DependencyConstraint?) =
        when (constraint) {
            null,
            ...
            is ContainsOnly -> listOf(Var(varId(name, constraint.vId, constraint.tId), nextId++))
                ...
                labels.map { L(it.key, it.value, nextId++, constraint) } +
                F(
                    listOf(mutableListOf(Hole(constraint)), mutableListOf(Hole(constraint))),
                    nextId++,
                    constraint
                )
    ...
    fun step(): List<Map<String, ConcreteNode>> {
        ...
        state.forEach { (f, root) ->
            if (root is F)
                root.params.forEachIndexed { i, options ->
                    options.forEach {
                        it.enumerate(f, i)
                    } // assumes no top-level holes, a fair assumption...
                }
    ...
    fun Node.concretizations(): Sequence<ConcreteNode> =
        when (this) {
            is F ->
                if (params.isEmpty()) sequenceOf(ConcreteF(listOf()))
                else if (params.any { it.any { it is Hole } }) emptySequence()
                else
                    lazySeqCartesianProduct(
                        params.map { it.asSequence().flatMap { it.concretizations() } })
                        .map { ConcreteF(it.toList()) }
            is L ->
                if (params.isEmpty()) sequenceOf(ConcreteL(this.label, listOf()))
                else if (params.any { it.any { it is Hole } }) emptySequence()
                else
                    lazySeqCartesianProduct(
                        params.map { it.asSequence().flatMap { it.concretizations() } })
    ...
    private fun Node.holelessCopy(): Node? =
        when (this) {
            is F -> {
                ...
                if (p.any { it.isEmpty() }) null else F(p, this.id, this.constraint)
            }
            is L -> {
                val p = params.map { it.mapNotNull { it.holelessCopy() }.toMutableList() }
                if (p.any { it.isEmpty() }) null else L(label, p, this.id, this.constraint)
            }
            is Var -> this
            is Hole -> null
        }
    ...
    private fun contexts(pos: List<Example>, neg: List<Example>): Set<Map<String, ConcreteNode>> {
        ...
        //  Rightmost param of F can't be fresh even if it's a HOF and parent allows - think about
        // this more
        //  If last param is a label L<a->b> don't want to erroneously say a can be fresh just bc
        // it's on the left
        ...
        val possTys =
            (concreteOptions as Map<String, Node>).map { (n, t) ->
                when (t) {
                    is F -> {
                        ...
                        // TODO simplify this with concretizations()
                            ...
                            lazyCartesianProduct(
                                t.params.mapIndexed { i, options ->
                                    options
                                        .flatMap { it.concretizations() }
                                        .filter { node ->
                                            when (val c = options.first().constraint) {
                                                is MustContainVariables -> {
                                                    val vars = node.vars()
                                                    c.vars.all {
                                                        (varId(n, it.first, it.second)) in vars
                                                    }
                                                }
                                                is ContainsOnly -> {
                                                    val vars = node.vars()
                                                    vars.size == 1 &&
                                                            vars.single() == varId(n, c.vId, c.tId)
                                                }
                                                else -> true
                                            }
                                        }
        ...
        return lazySeqCartesianProduct(possTys)
            .map { concreteOptions.keys.zip(it).toMap() }
            .filter {
    ...
    fun Node.enumerate(name: String, param: Int, inLabel: Boolean = false): Unit =
        when (this) {
    ...
    fun applyBinding(t: ConcreteNode, varId: Int, sub: ConcreteNode): ConcreteNode {
    ...
    private fun instantiate(n: ConcreteNode): ConcreteNode =
        when (n) {
            ...
            is ConcreteVar -> n
            // TODO This is not actually doing anything but it should take the same amount of time
            // as a real instantiation pass so it's ok just here as proof of concept
            else -> TODO("Try skipping nullaries then fast forwarding at the end")
        }

    fun type(context: Map<String, ConcreteNode>, example: Example): ConcreteNode? =
        when (example) {
            is Name -> context[example.name]?.let { instantiate(it) } // context[example.name]
            is App ->
                type(context, example.fn).let { f ->
...
data class ConcreteF(val params: List<ConcreteNode>) : ConcreteNode {
    ...
    override fun toString(): String =
        params.joinToString(separator = "->") { if (it is ConcreteF) "($it)" else "$it" }
}
