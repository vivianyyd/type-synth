...
class UnificationTest {
     ...
     * TODO I think this note generated by copilot is wrong lol
     */
    @Test
    fun `unify function where both param and arg specialize: concrete in parameter`() {        
        ...
        val unify = OneUnification(context, listOf(example))
        assertTrue(unify.ok(), "OneUnification fails here due to strict occurs check (known limitation)")
    }
     ...
     */
    @Test
    fun `unify function where both param and arg specialize: concrete in argument`() {
        val context = SearchState(
            mapOf("f" to 0, "g" to 1),
            listOf(
                // f: ('a -> 'a -> 'a) -> Int
                Arrow(
                    Arrow(Variable(0), Arrow(Variable(0), Variable(0))), // 'a -> 'a -> 'a
                    NamedLabel(0, listOf())
                ),
                // g: 'a -> 'a -> Int
                Arrow(Variable(0), Arrow(Variable(0), NamedLabel(0, listOf())))
